...
const IS_LOCAL_DEV =
  import.meta.env.DEV || !import.meta.env.VITE_AWS_COGNITO_IDENTITY_POOL_ID;

// Mock health events data for local development
const MOCK_HEALTH_EVENTS = [
  {
    ...
    description:
      "We are investigating connectivity issues affecting some EC2 instances in the US-East-1 region.",
  },
  {
    ...
    description:
      "Scheduled maintenance for RDS instances in US-West-2. Some instances may experience brief downtime.",
  },
  {
    ...
    eventScopeCode: "PUBLIC",
    description:
      "Resolved: S3 service degradation in EU-West-1 has been resolved. All services operating normally.",
  },
  {
    ...
    eventScopeCode: "PUBLIC",
    description:
      "Resolved: Increased error rates for Lambda functions in US-East-1 have been resolved.",
  },
  {
    ...
    eventScopeCode: "ACCOUNT_SPECIFIC",
    description:
      "Security notification: Please review your ECS task definitions for potential security improvements.",
  },
];

// AWS SDK clients (only used in production)
let healthClient = null;
if (!IS_LOCAL_DEV) {
  const { HealthClient } = await import("@aws-sdk/client-health");
  ...
  const AWS_CONFIG = {
    ...
    credentials: fromCognitoIdentityPool({
      clientConfig: { region: import.meta.env.VITE_AWS_REGION || "us-east-1" },
      identityPoolId: import.meta.env.VITE_AWS_COGNITO_IDENTITY_POOL_ID,
    }),
  };

  healthClient = new HealthClient(AWS_CONFIG);
}

// Health Events API wrapper
export const healthEventsClient = {
  ...
  async getEvents(filters = {}) {
    ...
    try {
      const { DescribeEventsCommand } = await import("@aws-sdk/client-health");
      const command = new DescribeEventsCommand({
        filter: {
          regions: filters.regions || [],
          services: filters.services || [],
          eventTypeCategories: filters.eventTypeCategories || [],
          eventTypeCodes: filters.eventTypeCodes || [],
          eventStatusCodes: filters.eventStatusCodes || ["open", "upcoming"],
          startTimes: filters.startTimes || [],
          endTimes: filters.endTimes || [],
        },
  ...
  async getEventDetails(eventArns) {
    ...
    try {
      const { DescribeEventDetailsCommand } = await import(
  ...
  async getAffectedEntities(eventArn) {
    if (IS_LOCAL_DEV) {
      ...
      const mockEntities = {
        ...
        RDS: [
          {
            entityArn: "arn:aws:rds:us-west-2:123456789012:db:production-db-1",
            entityValue: "production-db-1",
          },
        ],
        S3: [
          {
            entityArn: "arn:aws:s3:::production-bucket-1",
            entityValue: "production-bucket-1",
          },
          {
            entityArn: "arn:aws:s3:::backup-bucket-2",
            entityValue: "backup-bucket-2",
          },
        ],
      };
    ...
    try {
      const { DescribeAffectedEntitiesCommand } = await import(
        "@aws-sdk/client-health"
      );
      ...
      console.error("Error fetching affected entities:", error);
      return [];
    }
  },

  // Mock auth for now (replace with your authentication system)
  auth: {
    async me() {
      return {
        id: "user123",
  ...
  entities: {
    Incident: {
      async list(sort = "-startTime", limit = 100) {
        const events = await healthEventsClient.getEvents({
          maxResults: limit,
        });
